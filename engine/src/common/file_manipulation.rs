...
pub fn load_text_file(file_path: &Path) -> Result<String, FileError> {
    ...
    File::open(file_path).and_then(|mut f| f.read_to_string(&mut buf))?;
...
pub fn load_binary_file(file_path: &Path) -> Result<Vec<u8>, FileError> {
    let mut buf = Vec::new();
    File::open(file_path).and_then(|mut f| f.read_to_end(&mut buf))?;
...
pub fn save_texture(tex: &Texture2d, path: &Path) -> Result<(), FileError> {
    ...
    let img_buf =
        image::ImageBuffer::<image::Rgba<u8>, _>::from_raw(img.width, img.height, img.data)
...
#[derive(Debug, Fail)]
pub enum FileError {
    #[fail(display = "No such file or directory: '{}'", _0)] FileNotFound(String),
    #[fail(display = "Not a file: '{}'", _0)] NotAFile(String),
    ...
    #[fail(display = "{}", _0)] ImageError(#[cause] image::ImageError),
}
